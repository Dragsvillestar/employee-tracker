...
function codeVerify() {
    ...
    confirmationResult.confirm(code)
        .then(async (userCredential) => {
            ...
            const managerUid = idTokenResult.claims.managerUid;
            ...
            setTimeout(() => {
                window.location.href = `/worker/home?username=${encodeURIComponent(user.displayName)}`;
            }, 1000);
        })
        .catch((error) => {
...
function signIn() {
    ...
    auth.signInWithEmailAndPassword(email, password)
        .then(async (userCredential) => {  // ✅ Use async to handle the token fetch
            ...
            console.log("Manager's UID:",managerUid)
            document.getElementById("message").textContent = "✅ Sign-in successful!";
